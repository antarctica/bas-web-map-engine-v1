/**
 * Note: file not exported from ol 3.1 - needs replacing on every ol upgrade :-(
 * Either remake all the modifications below, or create a non-standard API via goog.exportProperty declarations
 * in the official code - neither is very satisfactory!  Once ol3 versions are more stable these methods may
 * become part of the API
 * 
 * Minor modifications made 05/01/15:
 * - all occurrences of goog.isDef eliminated (replaced by simple test for empty, lines 62/63)
 * - all occurrences of goog.* replaced by magic.modules.Common.*
 */
...
ol.Ellipsoid.prototype.vincenty =
  ...
  var minDeltaLambda = opt_minDeltaLambda ? opt_minDeltaLambda : 1e-12;
  var maxIterations = opt_maxIterations ? opt_maxIterations : 100;
  var f = this.flattening;
  var lat1 = magic.modules.Common.toRadians(c1[1]);
  var lat2 = magic.modules.Common.toRadians(c2[1]);
  var deltaLon = magic.modules.Common.toRadians(c2[0] - c1[0]);
  ...
  return {
    ...
    initialBearing: magic.modules.Common.toDegrees(alpha1),
    finalBearing: magic.modules.Common.toDegrees(alpha2)
  };
};
